...
export default function ScreenShoter(monitor: gdk.monitor) {
  function TakeScreenShot(mode: string) {
    if (mode === "full") {
      execAsync(["bash", "-c", "hyprshot -m active -m output -o ~/Pictures/Screenshots"])
    } else if (mode === "region") {
  ...
  return <window
    ...
    <box vertical className="main">
      ...
      <box>
        <button onClicked={() => TakeScreenShot("full")}>
          <box vertical>
            <icon icon="screenshot-full-symbolic" />
            <label label="Full" />
          </box>
        </button>
        <button onClicked={() => TakeScreenShot("window")}>
          <box vertical>
            <icon icon="screenshot-window-symbolic" />
            <label label="Window" />
          </box>
        </button>
        <button onClicked={() => TakeScreenShot("region")}>
          <box vertical>
